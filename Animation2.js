...
function render() {
  ...
  // Setup Camera - follow the logo during translation
  const cameraTarget = vec3(translateX, translateY, 0);
  const V = lookAt(vec3(1, 4, 9), cameraTarget, vec3(0, 1, 0));
...
function defaultAnim() {
  switch (animSeq) {
    ...
    case 5: // Move to middle upper
      translateY += 0.02;
      if (translateY >= 2.5) animSeq = 6;
      break;

    case 6: // Move to top right
      translateX += 0.02;
      if (translateX >= 3.0) animSeq = 7;
      break;

    case 7: // Move to bottom right
      translateY -= 0.02;
      if (translateY <= -2.5) animSeq = 8;
      break;

    case 8: // Move to bottom left
      translateX -= 0.02;
      if (translateX <= -3.0) animSeq = 9;
      break;

    case 9: // Move to top left
      translateY += 0.02;
      if (translateY >= 2.5) animSeq = 10;
      break;

    case 10: // Move back to middle upper
      translateX += 0.02;
      if (translateX >= 0) animSeq = 6;
      break;
  }
}
